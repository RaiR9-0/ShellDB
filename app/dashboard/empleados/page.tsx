...
export default function EmpleadosPage() {
  ...
  const openNew = useCallback(() => { setForm(emptyForm); setEditing(null); setError(""); setOpen(true) }, [])
  ...
  return (
    <div className="p-6">
      <PageHeader title="Empleados" subtitle="Gestion del personal" actionLabel="Agregar" onAction={openNew} />
        ...
        <DialogContent className={styles.dialog.content}>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <Input value={form.codigo} onChange={(e) => setForm({ ...form, codigo: e.target.value })} disabled={!!editing} className={styles.input.default} />
            </div>
            <div className="flex flex-col gap-1.5">
              <Label className={styles.label}>Nombre</Label>
              <Input value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })} className={styles.input.default} />
            </div>
            <div className="flex flex-col gap-1.5">
              <Label className={styles.label}>Puesto</Label>
              <Input value={form.puesto} onChange={(e) => setForm({ ...form, puesto: e.target.value })} className={styles.input.default} />
            </div>
            <div className="flex flex-col gap-1.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                <Input value={form.telefono} onChange={(e) => setForm({ ...form, telefono: e.target.value })} className={styles.input.default} />
              </div>
              <div className="flex flex-col gap-1.5">
                <Label className={styles.label}>Salario</Label>
                <Input type="number" value={form.salario} onChange={(e) => setForm({ ...form, salario: e.target.value })} className={styles.input.default} />
              </div>
            </div>
            <div className="flex flex-col gap-1.5">
              <Label className={styles.label}>{editing ? "Nueva Clave (vacio = mantener)" : "Clave de Venta *"}</Label>
              <Input type="password" value={form.clave} onChange={(e) => setForm({ ...form, clave: e.target.value })} placeholder={editing ? "Dejar vacio para mantener" : "Minimo 4 caracteres"} className={styles.input.default} />
              <p className="text-xs text-[#7F8C8D]">Esta clave sera requerida para autorizar ventas.</p>
            </div>
